...
source /hostdata/scripts/jetson_variables.sh
...
cp /hostdata/cudnn.h /usr/include/
...
time cmake -D CMAKE_BUILD_TYPE=RELEASE \
      ...
      -D CUDNN_LIBRARY=/usr/lib/aarch64-linux-gnu/libcudnn.so.7 \
...
# copy deb files to host
if [[ $IMPORT_CHECK == *$OPENCV_VERSION* ]]; then
  cp /root/opencv/build/OpenCV-${OPENCV_VERSION}* /hostdata/opencv-aarch64-deb/
  echo "OpenCV Package Files copied to opencv-aarch64-deb folder"
fi
